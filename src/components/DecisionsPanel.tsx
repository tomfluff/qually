...
const KIND_LABEL: Record<Decision["kind"], string> = {
  ...
  keep: "kept", park: "set aside", unpark: "brought back", promote: "to code more", dismiss: "dismissed",
};
...
export function DecisionsSide({ hideUndone, setHideUndone }: {
...
export function DecisionsList({ hideUndone }: { hideUndone: boolean }) {
  const ledger = useStore((s) => s.ledger);
  const codebook = useStore((s) => s.codebook);
  const [copied, setCopied] = useState(false);
  const para = useMemo(() => methodsParagraph(ledger, Object.keys(codebook)), [ledger, codebook]);
  const rows = useMemo(
    () => ledger.map((d, i) => ({ d, i })).filter(({ d }) => !(hideUndone && d.undone)).reverse(),
    [ledger, hideUndone]);
